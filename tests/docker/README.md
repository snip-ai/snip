...
| **B — lifecycle** | `phase-b-lifecycle.test.mjs` | The plugin self-install path: `snip-bootstrap.sh` (and `snip-run.sh update-check`) download + verify + install the binary from a loopback fake release server; the `.uninstalled` marker keeps `snip uninstall` from being undone (`update-check` dormant, `update` reactivates); first-install / dormant / reactivation banners are emitted as the right channel (JSON `systemMessage` vs plain text). |
| **B — lifecycle banner** | `phase-b-lifecycle-banner.test.mjs` | Through the **real `claude`** + a mock Anthropic server: a pending `.lifecycle` event surfaces as a `systemMessage` that is **absent from every request sent to the model**, and the sentinel is consumed exactly once. Codifies the user-visible / model-invisible split. |
| **B — shell setup** | `phase-b-shell-setup.test.mjs` | The `/snip shell-setup` script: against a throwaway `HOME`, it writes one marked PATH block to the shell rc, is idempotent, `remove`s it cleanly, and rejects an unknown action (exit 2). No `claude`, no network. |
